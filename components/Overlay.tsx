...
export const Overlay: React.FC<OverlayProps> = ({ activeElement, isFinale }) => {
  // Use local state to strictly control the visibility timing of finale elements
  const [hasTriggeredFinale, setHasTriggeredFinale] = useState(false);

  useEffect(() => {
    if (isFinale) {
      // Small delay to allow the scene transition to begin before fading in UI
      const timer = setTimeout(() => setHasTriggeredFinale(true), 50);
      return () => clearTimeout(timer);
    } else {
      setHasTriggeredFinale(false);
    }
  }, [isFinale]);
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-30 overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        {/* Only show element-specific backgrounds if not in finale */}
        {!isFinale && Object.entries(ELEMENT_BACKGROUNDS).map(([key, url]) => (
          <div
            key={key}
            className={`absolute inset-0 transition-all duration-[1500ms] ease-in-out bg-cover bg-center ${
              activeElement === key ? 'opacity-30 scale-105' : 'opacity-0 scale-100'
            }`}
            style={{ 
        ...
        {/* Finale Main Background - Strictly conditional to avoid any start-screen leaks */}
        {isFinale && (
          <div 
            className={`absolute inset-0 bg-cover bg-center transition-opacity duration-[3000ms] ease-out ${
              hasTriggeredFinale ? 'opacity-100' : 'opacity-0'
            }`}
            style={{ 
        ...
        <div 
          className={`absolute inset-0 bg-black/40 transition-opacity duration-[3000ms] ${
            isFinale && hasTriggeredFinale ? 'opacity-100' : 'opacity-0'
          }`}
        />
      </div>

      {/* Cinematic Overlays: Noise, Vignette */}
      <div className="absolute inset-0 z-[1] opacity-[0.15] mix-blend-overlay bg-[url('https://grainy-gradients.vercel.app/noise.svg')] bg-repeat pointer-events-none" />
      <div 
      ...
      {/* Editorial Finale Text Layout - Wrapped in strict conditional */}
      {isFinale && (
        <div 
          className={`absolute inset-0 z-[40] flex flex-col items-center justify-center transition-all duration-[4000ms] ease-out ${
            hasTriggeredFinale ? 'opacity-100 translate-y-[-10vh]' : 'opacity-0 translate-y-0'
          }`}
        >
      ...
      {/* Footer Branding - Wrapped in strict conditional */}
      {isFinale && (
        <div 
          className={`absolute inset-x-0 bottom-6 md:bottom-10 z-[40] flex flex-col items-center gap-3 transition-all duration-[4000ms] ease-out delay-[2000ms] ${
            hasTriggeredFinale ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
          }`}
        >
